...
- **Release metadata is aligned at 0.2.5**: `pyproject.toml`, the runtime package, README, and
  all three provider manifests agree. The manifest test guards this agreement.
- Kimi's Windows launcher resolves `uv` from its standard user install path before falling back to
  `PATH`, then places its project environment under Kimi's cache. This avoids both the inherited-PATH
  failure and stale managed-copy `.venv` links.
...
The version-and-tag baseline is met at `v0.2.5`, and CI defines the local compile, lint, and pytest
